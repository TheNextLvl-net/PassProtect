...
public class DatabaseStorage implements DataStorage {
    ...
    @Override
    public List<Account> getAccounts(String email) throws SQLException {
        return executeQuery("SELECT * FROM accounts WHERE email = ?", this::transformRows, email);
    }

    @Override
    public Account getAccount(String email, String username) throws SQLException {
        return executeQuery(
                "SELECT * FROM accounts WHERE email = ? AND username = ? LIMIT 1",
                this::transformRow, email, username
        );
    }

    @Override
    public boolean accountExists(String email, String username) throws SQLException {
        return executeQuery(
                "SELECT EXISTS (SELECT 1 FROM accounts WHERE email = ? AND username = ? LIMIT 1) AS entry_exists",
                resultSet -> resultSet.getBoolean("entry_exists"), email, username
        );
    }
    ...
    public void createAccount(Account account) throws SQLException {
        executeUpdate("""
                ...
                account.uuid().toString(), account.email(), account.username(),
    ...
    @Override
    public void updateUsername(Account account) throws SQLException {
        executeUpdate(
                "UPDATE accounts SET username = ? WHERE uuid = ?",
                account.username(), account.uuid().toString()
        );
    }
